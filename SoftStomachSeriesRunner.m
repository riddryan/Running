classdef SoftStomachSeriesRunner < Runner
    ...
    properties
        %Mass
        mpelvis = .85; msoftstomach = .05;  mfoot=0; msoftseries=.1;
        
        %Leg and soft stomach stuff

        cleg=0; csoftstomach=0;
        lleg=1; lsoftstomach=.2;
        gslope = 0;
        g = 1;
        ...
        %Soft tissue in series stuff
        lsoftseries=.4;
        kleg=18; ksoftstomach=15;
        ksoftseries = 36; csoftseries=0;
    end
...
methods
    ...
    function [this] = SoftStomachSeriesRunner(input)
        %%
        this = this@Runner();
    ...
    function plot(this, state, varargin)
      ...
      numcoils=3;
      springwidth=.07;
      plotter.plotSpring(points.softseries(1),points.softseries(2),...
      ...
      plotter.plotSpring(points.pelvis(1),points.pelvis(2),...
          points.softstomach(1),points.softstomach(2),...
      ...
      plotter.plotSpring(points.foot(1),points.foot(2),...
          points.softseries(1),points.softseries(2),...
          numcoils,this.lsoftseries,springwidth,...
          'Color',softcolor) %soft series spring
      
      %Draw Masses
      plotter.plotMass(points.pelvis);
      plotter.plotMass(points.softstomach,'scaling',this.msoftstomach/this.mpelvis);
      plotter.plotMass(points.softseries,'scaling',this.msoftseries/this.mpelvis);
...
%               points = this.getPoints(thisState);
    ...
    function [xnew] = switchLegs(this,x,landangle)
       ...
       newstatestruc = SoftStomachSeriesRunnerState(x);
    ...
    function [speed] = getSpeed(this, x0, xf, tf)
        ...
        x0struc = SoftStomachSeriesRunnerState(x0);
        xfstruc = SoftStomachSeriesRunnerState(xf);
...
points.softseries(2) = q5 + q2*s1; 
    ...
    function [newstate] = GoodInitialConditions(this,x0)
      ...
      upelvx = -(Jc(1,1)*u1 + Jc(1,2)*u2 + Jc(1,6)*u6);
      upelvy = -(Jc(2,1)*u1 + Jc(2,2)*u2 + Jc(2,6)*u6);
      
      newstate=x0;
      newstate([4 5 10 11]) = [pelvx pelvy upelvx upelvy];
