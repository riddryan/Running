classdef NonlinParSoftRunner < Runner
...
%                 ParForce(i,1) = r.ksoftparallel*(x(3) - r.lsoftparallel) + r.csoftparallel*x(8);
            ...
            plot(allt,GRF)
            hold on
            pt1 = find(kparhistory~=r.kimpact,1);
            pt2 = find(kparhistory==r.kstance,1);
    ...
    methods
        ...
        function [this] = NonlinParSoftRunner(input)
            %%
            this = this@Runner();
        ...
        function plot(this, state, varargin)
            ...
            statestruc = NonlinParSoftRunnerState(state);
            ...
            totalmass = this.mpelvis + this.msoftparallel;
            massrange = maxmass-minmass;
            
            %Draw Masses
            plotter.plotMass(offsetpelvis,'scaling',this.mpelvis/totalmass);
            if this.msoftparallel>.05
                plotparmass = this.msoftparallel/totalmass;
            else
                plotparmass = .05;
            end
            
            plotter.plotMass(parpar,'scaling',plotparmass);
            %Draw Foot
        ...
        function ksoftparallel = getksoftparallel(this,state)
            ...
            if legpos/this.lleg >= pt1 && legvel<=0  %Impact phase
                ksoftparallel  = this.kimpact;
            elseif legpos/this.lleg <= pt1 && legpos/this.lleg >= pt2 && legvel<=0 % Transition Region
                ksoftparallel  = interp1([pt1 pt2],[this.kimpact this.kstance],legpos); %linear interpolation
            else %We are past the transition region
                ksoftparallel  = this.kstance;
            end
        end
        ...
        function [value, isTerminal, direction] = LiftOrFall(this,t,state)
            ...
            %Falling
            [value2, isTerminal2, direction2] = Fall(this,t,state);
        ...
        function [xnew] = switchLegs(this,x,landangle)
            ...
            newstatestruc = NonlinParSoftRunnerState(x);
        ...
        function [speed] = getSpeed(this, x0, xf, tf)
            ...
            x0struc = NonlinParSoftRunnerState(x0);
            xfstruc = NonlinParSoftRunnerState(xf);
        ...
        function [MM,rhs] = getMMandRHS(this,time,state)
            ...
            this.getParams();
            this.getQandUdefs(state);
            ksoftparallel = this.getksoftparallel(state);
...
%             newstate = [x0(1:5) unew'];
        ...
        function [c,ceq] = TakeoffConstraints(this,x0,xf,tf,allx,allt,tair)
            ...
            statesair = NonlinParSoftRunnerState(xair);
        ...
        function [c,ceq] = nonlinspringconstraints(this)
            c = [this.transitionend - this.transitionstart; this.kstance - this.kimpact];
            ceq = [];
        end
...
+ msoftparallel,-1);
        ...
        function parallelPower = getParallelPower(this,x)
            sz = NonlinParSoftRunnerState(x);
            force = -this.getksoftparallel(x)*(sz.softparallel.stretch - this.lsoftparallel)...
