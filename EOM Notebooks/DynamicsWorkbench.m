...
Options[formtrigreplacements]={TrigReplacements->{{Sin,"s"},{Cos,
            "c"},{Tan,"t"},{Sec,"sec"},{Csc,"csc"}},
      ArgReplacements->None };
...
variables. The shortened variables are given as strings. For example, 
  an equation M*g*Sin[q[1]]*Cos[q[2]]*Sin[q[1]-q[2]]*
      Sin[unknown] returns {Sin[q[1]]->"s1",Cos[q[2]]->"c2",
        ...
        ArgReplacements->{{gamma,"g"},{alpha,
          "a"}} specifies additional trig arguments that can be shortened, 
  and TrigReplacements->{{Sin,"s"},{Cos,
...
formtrigreplacements[eom_,opts___]:=
  Module[{trigreps,altargs,alltrig,trigreplaced,argreplacements,argtostrings,
      ...
      reorder},
    trigreps=
      TrigReplacements/.{opts}/.Options[formtrigreplacements]/.None->{};
    altargs=
      ArgReplacements/.{opts}/.Options[formtrigreplacements]/.None->{};
    ...
    trigreplaced=Map[Head,alltrig]/.Apply[Rule,trigreps,{1}];
      ...
      starting with q[n_]:>ToString[n], 
      followed by alternative arg replacements if any (whether in vector or \
matrix form) e.g. {q[n_]:>ToString[n],gamma->"g"} *)
    argreplacements=
      Prepend[
        Apply[Rule,
          Replace[altargs/.None->{},x:{_,_String}->{x}],{1}],
        q[n_]:>ToString[n]];
    ...
    plusminustostrings=
      argstostrings/.(x_String - y_String):>(x <> "m" <> 
                y)/.(x_String + y_String) :>( x <> "p" <> y);
    ...
    (* Use the two lists to form a set of rules, {Sin[q[1]]->"s1",
        Cos[q[2]]->"c2"} *)
    trigSubs=
      MapThread[
...
Options[ExportExpression]={qusubs->Automatic,TrigSubs->Automatic,
      psub->{(x_)^2->HoldForm[x*x]}};
ExportExpression[Mx_?SymmetricQ, name_String,opts___]:=
    Module[{elementsreplaced,quSubs,trigSubs,pSub},
      quSubs=
        qusubs/.{opts}/.Options[ExportExpression]/.Automatic->
            formqureplacements;
      trigSubs=
        TrigSubs/.{opts}/.Options[
                ExportExpression]/.None->{}/.Automatic->
...
ExportExpression[rhs_List?VectorQ, name_String,opts___]:=
    Module[{elementsreplaced,quSubs,trigSubs,pSub},
      quSubs=
        qusubs/.{opts}/.Options[
              ExportExpression]/.Automatic->formqureplacements;
      trigSubs=
        TrigSubs/.{opts}/.Options[
                ExportExpression]/.None->{}/.Automatic->\
...
Options[ExportCode]={OutputFile->None,Forces->None,
      Expressions->None};
...
ExportCode[{mass_List?MatrixQ,rhs_List?VectorQ},opts___]:=
    Module[{ostrm,outputfile,quSubs,trigSubs,linearforceterms,forces,
      ...
      expressions=
        Replace[Expressions/.{opts}/.Options[ExportCode]/.None->{}, 
          x:{_,_String}->{x}];
      ...
      myopts=Sequence[qusubs->quSubs,TrigSubs->trigSubs]; (* 
      ...
      WriteString[ostrm,StringReplace[ (* strip out the quote marks *)
          ...
          StringJoin[
            Map[ToString[
                  StringForm["`1` = `2`; ", #[[2]],
                    CForm[#[[1]]/.quSubs/.{Sin->"sin",Cos->"cos",
                          Tan->"tan",Sec->"sec",
                          Csc->"csc"}]]]&,trigSubs]],"\""->""],"\n\n"];
      ...
      WriteString[ostrm,
        ToString[StringForm["xdot = [x(`1`+1:`2`); udot];",Length[qdofs],2*Length[qdofs]]],"\n"];
