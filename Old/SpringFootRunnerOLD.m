...
%         achillesangle = 138/360*2*pi;
        ...
        phases = {'Heel' 'HeelToe' 'Toe' 'Aerial'};
    end
    ...
    methods (Static)
        ...
        function [] = test()
            ...
            aviname = [savepath 'testSFoot.avi'];
...
%       end
      ...
      plotter.plotSpring(points.heel(1),points.heel(2),...
          points.pelvis(1),points.pelvis(2),...
          numcoils,this.achilleslength,springwidth,'Color',[0 1 1]) %achilles spring
    ...
    function [xf,tf,allx,allt,tair,phasevec,this]= onestep(this, x0,varargin)
        %%
        RelTol = 1e-6; %10; %
        AbsTol = 1e-6; %10; %
        tmax = 5; %2; %6;
      ...
      %% Heel Phase
      phase = phases{1};
      opts = odeset('Events', @(t,x) this.ToeOnOrFall(t,x),'RelTol',RelTol','AbsTol',AbsTol);
      ...
      allt = [allt;t]; allx = [allx;x]; phasevec = [phasevec; 3*ones(length(t),1)];
      %% Aerial Phase
      tair = allt(end);
      phase = phases{4};
      [x0,~] = this.phaseTransition(allt(end),allx(end,:),phase);
      ...
      allt = [allt; t];
      allx = [allx;x];
      phasevec = [phasevec; 4*ones(length(t),1)];
      ...
      [xf,impulses] = this.phaseTransition(tf,xf,'Heel');
    ...
    function [value, isTerminal, direction] = ToeOnOrFall(this,t,state)
        points = this.getPoints(state);
...
%        newstatestruc.stanceLeg.Length = this.lleg;
    ...
    function [speed] = getSpeed(this, x0, xf, tf)
        ...
        x0struc = SoftStomachRunnerState(x0);
        xfstruc = SoftStomachRunnerState(xf);
...
rhs(3) = q4*(-2*u3*u4*(mfoot + mheel + mtoe) - g*mfoot*cos(q3 - gslope) - ...
g*mheel*cos(q3 - gslope) - g*mtoe*cos(q3 - gslope) - s3m5*ltoe*mtoe*(u5*u5) - ...
s3m6*lheel*mheel*(u6*u6) + s3m6*lheel*power(2*c3m6*q4*lheel + q4*q4 + ...
lheel*lheel,-0.5)*(-(achilleslength*kachilles) + ...
kachilles*power(2*c3m6*q4*lheel + q4*q4 + lheel*lheel,0.5) + ...
cachilles*power((c3*q4*u3 + s3*u4 + c6*u6*lheel)*(c3*q4*u3 + s3*u4 + ...
c6*u6*lheel) + (q4*s3*u3 - c3*u4 + s6*u6*lheel)*(q4*s3*u3 - c3*u4 + ...
s6*u6*lheel),0.5))); 
rhs(4) = u4*cleg + kleg*(-q4 + lleg) + q4*(mfoot + mheel + mtoe)*(u3*u3) + ...
c3m5*ltoe*mtoe*(u5*u5) + c3m6*lheel*mheel*(u6*u6) + q4*kachilles*(-1 + ...
achilleslength*power(2*c3m6*q4*lheel + q4*q4 + lheel*lheel,-0.5)) - ...
q4*cachilles*power(2*c3m6*q4*lheel + q4*q4 + ...
lheel*lheel,-0.5)*power((c3*q4*u3 + s3*u4 + c6*u6*lheel)*(c3*q4*u3 + s3*u4 + ...
c6*u6*lheel) + (q4*s3*u3 - c3*u4 + s6*u6*lheel)*(q4*s3*u3 - c3*u4 + ...
s6*u6*lheel),0.5) - c3m6*lheel*power(2*c3m6*q4*lheel + q4*q4 + ...
lheel*lheel,-0.5)*(-(achilleslength*kachilles) + ...
kachilles*power(2*c3m6*q4*lheel + q4*q4 + lheel*lheel,0.5) + ...
cachilles*power((c3*q4*u3 + s3*u4 + c6*u6*lheel)*(c3*q4*u3 + s3*u4 + ...
c6*u6*lheel) + (q4*s3*u3 - c3*u4 + s6*u6*lheel)*(q4*s3*u3 - c3*u4 + ...
s6*u6*lheel),0.5)) - g*mfoot*sin(q3 - gslope) - g*mheel*sin(q3 - gslope) - ...
g*mtoe*sin(q3 - gslope); 
...
rhs(6) = (u5 - u6)*cfoot - (-q5 + q6 + footangle)*kfoot - ...
2*c3m6*u3*u4*lheel*mheel - g*lheel*mheel*cos(q6 - gslope) + ...
q4*s3m6*lheel*mheel*(u3*u3) - q4*s3m6*lheel*power(2*c3m6*q4*lheel + q4*q4 + ...
lheel*lheel,-0.5)*(-(achilleslength*kachilles) + ...
kachilles*power(2*c3m6*q4*lheel + q4*q4 + lheel*lheel,0.5) + ...
cachilles*power(2*c3m6*q4*u3*u6*lheel + 2*s3m6*u4*u6*lheel + q4*q4*(u3*u3) + ...
u4*u4 + u6*u6*(lheel*lheel),0.5)); 
...
constraintJacobianHeelDot(2,6) = -(s6*u6*lheel); 
                ...
                C = constraintJacobianHeel;
                CDot = constraintJacobianHeelDot;
...
kleg*((-q4 + lleg)*(-q4 + lleg)) + kachilles*((achilleslength - ...
power(power(q4,2) + 2*c3m6*q4*lheel + power(lheel,2),0.5))*(achilleslength - ...
power(power(q4,2) + 2*c3m6*q4*lheel + power(lheel,2),0.5))) + ...
2*g*mpelvis*(q2*cos(gslope) - q1*sin(gslope)) - 2*g*mfoot*(-(q2*cos(gslope)) ...
- q4*sin(q3 - gslope) + q1*sin(gslope)) - 2*g*mtoe*(-(q2*cos(gslope)) - ...
q4*sin(q3 - gslope) - ltoe*sin(q5 - gslope) + q1*sin(gslope)) - ...
2*g*mheel*(-(q2*cos(gslope)) - q4*sin(q3 - gslope) - lheel*sin(q6 - gslope) + ...
q1*sin(gslope)))/2.;
...
(kleg*((q4 - lleg)*(q4 - lleg)))/2. + (kachilles*((-achilleslength + ...
power(power(q4,2) + 2*c3m6*q4*lheel + power(lheel,2),0.5))*(-achilleslength + ...
power(power(q4,2) + 2*c3m6*q4*lheel + power(lheel,2),0.5))))/2.;
...
points.toe(2) = q2 + q4*s3 + s5*ltoe; 
    ...
    function [newstate] = GoodInitialConditions(this,x0)
      ...
      stanceang = q3+pi;
      heelang = q6+pi;
      pelvx = q4*cos(stanceang)+this.ltoe*cos(heelang);
      pelvy = q4*sin(stanceang)+this.ltoe*sin(heelang);
      
      Jc = this.getConstraints(x0,'Heel');
      
      upelvx = -(Jc(1,3:6)*[u3;u4;u5;u6]);
      upelvy = -(Jc(2,3:6)*[u3;u4;u5;u6]);
      
      newstate=x0;
      newstate([1 2 7 8]) = [pelvx pelvy upelvx upelvy];
...
mpelvis + mtoe))*power(mfoot + mheel + mpelvis + mtoe,-1);
    ...
    function footPower = getFootPower(this,x)
        ...
        force = -this.kfoot*(sz.toe.Angle - sz.heel.Angle - this.footangle) ...
    ...
    function achillesPower = getAchillesPower(this,x)
        ...
        achillespos = points.pelvis - points.heel;
%         achillesvel = power((c3*q4*u3 + s3*u4 + c6*u6*lheel)*(c3*q4*u3 + s3*u4 + ...
%             c6*u6*lheel) + (q4*s3*u3 - c3*u4 + s6*u6*lheel)*(q4*s3*u3 - c3*u4 + ...
%             s6*u6*lheel),0.5);
        
        u = x(7:12)';
        J = this.getVelJacob(x);
        pelvvel(1:2,1) = J(1:2,:)*u;
        heelvel(1:2,1) = J(7:8,:)*u;
        achillesvel = pelvvel-heelvel;
        force = -this.kachilles*(norm(achillespos) - this.achilleslength)*achillespos'/norm(achillespos)...
                -this.cachilles*achillesvel;

%         force = -this.kachilles*(norm(achillespos) - this.achilleslength) ...
%                 -this.cachilles*achillesvel;
        velocity = achillesvel;
        achillesPower = dot(force,velocity);
    end
