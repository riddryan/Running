classdef SoftStomachRunner < Runner
    %Running Model with a spring leg, and a soft-stomach: a mass on a
    %spring above the pelvis
  
    properties
        mpelvis = .8; msoftstomach = .2; mfoot=0;
%         landangle = 2.028722624;
        gslope = 0;
        g = 1;
        N=10;
        statestovary = [1 3 6 7 8];
        statestomeasure = [3 6 7 8];
        cleg=0; csoftstomach=0;
        lleg=1; lsoftstomach=.2;
        kleg=12.9801; ksoftstomach=15;
    end
...
methods
    ...
    function [this] = SoftStomachRunner(input)
        %%
        this = this@Runner();
    ...
    function plot(this, state, varargin)
      ...
      statestruc = SoftStomachRunnerState(state);
...
%               points = this.getPoints(thisState);
    ...
    function [value, isTerminal, direction] = HeelStrikeOrFall(this,t,state,landangle)
        ...
        %Hitting the Ground
        swingfoot = points.pelvis(2) + sin(landangle)*this.lleg;
    ...
    function [xnew] = switchLegs(this,x,landangle)
       ...
       newstatestruc = SoftStomachRunnerState(x);
...
msoftstomach,-1); 
    ...
    function [newstate] = GoodInitialConditions(this,x0)
        ...
        pelvy = q2*sin(stanceang);
        
        Jc = this.getConstraints(x0,'stance');
        
        upelvx = -(Jc(1,1)*u1 + Jc(1,2)*u2);
        upelvy = -(Jc(2,1)*u1 + Jc(2,2)*u2);
        
        newstate=x0;
        newstate([4 5 9 10]) = [pelvx pelvy upelvx upelvy];
